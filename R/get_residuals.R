...
get_residuals<-function(data, condition_column, experimental_columns, response_column, condition_is_categorical,
  ...
  if(error_is_non_normal==FALSE){
    ...
    if(length(experimental_columns)==1){
      ...
      lmerFit <- lme4::glmer.nb(response_column ~  (1 | experimental_column1) + (1 | experimental_column2) + (1 | experimental_column3) + (1 | experimental_column4) + (1 | experimental_column5) + offset(log(total_column)) , data=Data, family=family_p)
    }
  }else{
    if(length(experimental_columns)==1){
