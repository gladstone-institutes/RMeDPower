...
#' @param family_p The type of distribution family to specify when the error probability distribution of response is non-normal. If family is "binomial" then binomial(link="logit") is used, if family is "poisson" then poisson(link="log") is used, if family is "poisson_log" then poisson(link="logit") is used.
...
calculate_lmer_estimates <- function(data, condition_column, experimental_columns, response_column, total_column = NULL, condition_is_categorical,
