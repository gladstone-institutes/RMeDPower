...
getEstimatesOfInterest <- function(data, design, model) {
  ...
  res <- list()
  ##get estimates
  res[[1]] <- calculate_lmer_estimates_covariate(data, condition_column, experimental_columns, response_column, total_column, condition_is_categorical, covariate,
                                                 crossed_columns, error_is_non_normal, family_p, na.action)
  ##visualize estimates
  res[[2]] <- get_residuals_covariate(data, condition_column, experimental_columns, response_column, total_column, condition_is_categorical, covariate,
                                                crossed_columns, error_is_non_normal, family_p, na.action)
